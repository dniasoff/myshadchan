...
-- Returns JSONB with one key per file table containing array of {id, filename, content_base64, content_type}.
...
begin
  ...
  -- Entity files (attachments)
  select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) into v_rows
  from (
    select
      ef.id,
      ef.original_name as filename,
      encode(ef.file_bytes, 'base64') as content_base64,
      ef.content_type
