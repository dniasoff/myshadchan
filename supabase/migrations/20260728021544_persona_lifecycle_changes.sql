alter table "public"."account_members" add constraint "account_members_status_check" CHECK ((status = ANY (ARRAY['active'::text, 'archived'::text]))) not valid;
...
-- `db diff` does not emit GRANT/REVOKE statements (documented pitfall,
-- 06_grants.sql's own header) — added by hand to match the source of truth.
