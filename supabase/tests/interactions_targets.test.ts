...
/**
 * Runs Story 3.5's interactions target-type widening suite against the
 * local Supabase stack.
 *
 * The assertions live in interactions_targets.sql, because what they check —
 * the widened target_type/scope_link constraints (AC 1), the target-aware
 * `scope = 'account'` RLS disjunct (AC 3), server-set actor attribution
 * (AC 4) and the two new purge triggers (AC 5) — only exists inside
...
// interactions_targets.sql wraps its own checks in `begin; ... rollback;`
// and clears public.account_members inside that transaction before setting
// up its own fixtures — the same isolation shape as every other suite in
// this directory.
...
describe("interactions target-type widening (database)", () => {
