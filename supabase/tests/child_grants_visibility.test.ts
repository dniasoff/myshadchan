...
/**
 * Runs the child_grants visibility guard against the local Supabase stack.
 *
 * The assertion lives in child_grants_visibility.sql because it can only be
 * expressed by reading the table as four different callers — proposer,
 * grantee, an unrelated household and a single-role member — which no mock
 * reproduces: the grant, the RLS policies and current_member_role() all have
 * to be simultaneously right for it to pass.
 *
...
describe("child_grants row visibility (database)", () => {
