...
select 'AC 4(d): enforce_household_scope is attached to exactly 14 tables, neither tasks nor interactions among them',
       (select count(*) from pg_trigger where tgfoid = 'public.enforce_household_scope'::regproc and not tgisinternal) = 14
   and not exists (
