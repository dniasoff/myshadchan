...
/**
 * Runs the single_notes RLS suite (Story 16.3 / FR69, PRV-4) against the local
 * Supabase stack.
 *
 * The assertions live in single_notes_rls.sql because what they check —
 * two RLS predicates deciding who reads a single's own notes — only exists
...
describe("single_notes_rls (database)", () => {
  ...
  it("runs every check group (single's own read, manager's shared-only read, cross-household, writes, helper/shadchan, toggle)", () => {
