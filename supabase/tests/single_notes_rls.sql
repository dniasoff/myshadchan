--
-- single_notes row-level security (Story 16.3 / FR69, PRV-4) — database suite.
--
...
--
-- The feature is one sentence: a single writes private notes in her own words
...
-- single_notes_rls.test.ts BEFORE this file: one household, one
-- parent_admin, and two `single` logins (Leah, Rivka) each linked to their own
-- singles row. This file adds Leah's two notes (one shared, one hidden),
...
values ('51830000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000000',
        'authenticated', 'authenticated', 'notes-outsider-parent@test.local');

insert into public.accounts (name, kind) values ('Notes Outsider Household', 'household')
...
-- ---------------------------------------------------------------------------
-- Arrange: Leah writes her own notes, through the policy, as herself.
...
insert into public.single_notes (single_id, body, visible_to_manager)
...
select 'CONTROL: all three note rows exist and Leah''s hidden row is flagged not-visible (without this, every negative below is vacuous)',
       (select count(*) from public.single_notes) = 3
  and (select visible_to_manager from public.single_notes where id = :leah_hidden_id) = false
  and (select visible_to_manager from public.single_notes where id = :leah_shared_id) = true;
...
insert into results (name, passed)
select 'Leah sees exactly her own two notes — visibility_to_manager does not hide a row from its author',
       (select count(*) from public.single_notes) = 2;

insert into results (name, passed)
select 'Leah sees her HIDDEN row (the flag governs the manager''s read, not hers)',
       exists (select 1 from public.single_notes where id = (select value::bigint from ids where name = 'leah_hidden_id'));

insert into results (name, passed)
select 'Leah does NOT see her sibling Rivka''s note — same household, same role, different single',
       not exists (select 1 from public.single_notes where id = (select value::bigint from ids where name = 'rivka_shared_id'));
...
insert into results (name, passed)
select 'parent_admin SEES Leah''s shared note',
       exists (select 1 from public.single_notes where id = (select value::bigint from ids where name = 'leah_shared_id'));

insert into results (name, passed)
select 'parent_admin does NOT see Leah''s hidden note — THE feature; the row exists (see CONTROL) and is withheld by visible_to_manager',
       not exists (select 1 from public.single_notes where id = (select value::bigint from ids where name = 'leah_hidden_id'));
...
insert into results (name, passed)
select 'A parent_admin in ANOTHER household sees no notes at all',
       (select count(*) from public.single_notes) = 0;

insert into results (name, passed)
select 'That outsider gets zero rows even when naming Leah''s SHARED row by id — the id is not an oracle',
       not exists (select 1 from public.single_notes where id = (select value::bigint from ids where name = 'leah_shared_id'));
...
begin
  ...
  begin
    ...
    values (public.current_context_id(),
            (select single_id from public.single_notes where id = v_single_id),
            'A manager putting words in her mouth.');
    insert into results values (
      'parent_admin CANNOT insert a note on a single''s behalf', false,
      'the insert succeeded; the manager policy is SELECT-only and policy 1''s with-check should have refused');
  exception when others then
    perform pg_temp.denied('parent_admin CANNOT insert a note on a single''s behalf', '42501', sqlstate, sqlerrm);
  end;
end;
$$;
...
begin
  ...
  update public.single_notes set body = 'rewritten by the manager' where id = v_id;
  get diagnostics v_rows = row_count;
  insert into results values (
    'parent_admin''s UPDATE of a shared note affects zero rows',
    v_rows = 0, format('row_count = %s', v_rows));

  delete from public.single_notes where id = v_id;
  get diagnostics v_rows = row_count;
  insert into results values (
    'parent_admin''s DELETE of a shared note affects zero rows',
    v_rows = 0, format('row_count = %s', v_rows));
end;
$$;
...
select 'CONTROL: after both attempts the shared row still exists with its original text (the two zero-row results above were refusals, not no-ops on a missing row)',
       (select body from public.single_notes where id = :leah_shared_id)
...
-- and writing a note into her sibling's record.
set local role authenticated;
set local request.jwt.claims = '{"sub":"51810000-0000-0000-0000-000000000002","role":"authenticated"}';
...
begin
  ...
  from public.single_notes
  where id = (select value::bigint from ids where name = 'rivka_shared_id');
  begin
    insert into public.single_notes (single_id, body)
    values (v_rivka_single_id, 'Leah writing into Rivka''s notes.');
    insert into results values (
      'A single CANNOT write a note naming another single''s single_id (same account, so the composite FK permits it — only the with-check refuses)',
...
-- ---------------------------------------------------------------------------
-- 6. Helper and shadchan see zero — they are not in the manager policy.
-- ---------------------------------------------------------------------------
reset role;

-- Switch the parent to helper role (fixture mutation: must run as postgres)
update public.account_members
set role = 'helper'
where user_id = '51810000-0000-0000-0000-000000000001';

set local role authenticated;
set local request.jwt.claims = '{"sub":"51810000-0000-0000-0000-000000000001","role":"authenticated"}';

insert into results (name, passed)
select 'helper role resolves correctly',
       public.current_member_role() = 'helper';

insert into results (name, passed)
select 'helper sees NO notes — not in manager policy (precedent: single_preferences)',
       (select count(*) from public.single_notes) = 0;

reset role;

-- STRUCTURAL: a shadchan membership cannot exist on a household account, so a
-- shadchan can never reach single_notes. The protection is enforced at the
-- membership layer by enforce_membership_role_matches_context, which is
-- STRONGER than an RLS policy because the membership itself cannot be created.
-- This replaces the prior assertion that tried to flip the role to 'shadchan'
-- and then check RLS — that path is impossible because the UPDATE is refused
-- by the trigger before any policy is evaluated.
do $$
begin
  update public.account_members
  set role = 'shadchan'
  where user_id = '51810000-0000-0000-0000-000000000001';
  insert into results values (
    'STRUCTURAL: a shadchan membership cannot exist on a household account, so a shadchan can never reach single_notes',
    false,
    'the UPDATE succeeded; enforce_membership_role_matches_context should have refused it with check_violation'
  );
exception when sqlstate '23514' then
  perform pg_temp.denied(
    'STRUCTURAL: a shadchan membership cannot exist on a household account, so a shadchan can never reach single_notes',
    '23514', sqlstate, sqlerrm
  );
end;
$$;

-- Restore the parent_admin role for subsequent tests (the helper->shadchan
-- flip was blocked by the trigger, so the role is still helper)
update public.account_members
...
-- ---------------------------------------------------------------------------
-- 7. Toggling visible_to_manager flips manager visibility.
-- ---------------------------------------------------------------------------
set local role authenticated;
set local request.jwt.claims = '{"sub":"51810000-0000-0000-0000-000000000002","role":"authenticated"}';

update public.single_notes
set visible_to_manager = true
where id = (select value::bigint from ids where name = 'leah_hidden_id');
...
insert into results (name, passed)
select 'After Leah toggles visible_to_manager=true, parent_admin NOW sees the previously hidden note',
       exists (select 1 from public.single_notes where id = (select value::bigint from ids where name = 'leah_hidden_id'));

insert into results (name, passed)
select 'parent_admin now sees three rows (both Leah notes + Rivka shared)',
       (select count(*) from public.single_notes) = 3;
