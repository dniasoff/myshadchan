...
-- sensitivity tier, no role check. Two logins inside the SAME household
-- account (a parent_admin and a single) both read and write the same row —
-- proving the absence of a role restriction, not merely its presence for one
-- role, which is the medical_notes.sql failure mode this suite deliberately
-- does NOT repeat. A third login, a parent_admin of a SECOND, unrelated
-- household account, sees zero rows (the account-scoping half). A fourth
-- login holds no account_members row at all (the fail-closed case).
--
...
-- ---------------------------------------------------------------------------
-- Arrange: four logins. u1 (parent_admin) and u2 (single) share household
-- account A — the pair that proves no role restriction. u3 is a parent_admin
-- of a SEPARATE household account B. u4 gets no account_members row anywhere
...
-- ---------------------------------------------------------------------------
-- (c) SELECT + UPDATE: single (u2), a DIFFERENT role in the SAME account,
-- also sees and can write the row — AC 6's "no role check", proven with a
-- real second role rather than only asserted in prose.
-- ---------------------------------------------------------------------------
set local role authenticated;
set local request.jwt.claims = '{"sub":"e7222222-2222-2222-2222-222222222222","role":"authenticated"}';

insert into results (name, passed)
select '(c) select: single (u2), a different role in the same account, also sees the external link (AC 6: no role check)',
       (select count(*) from public.shidduchim_external_links) = 1;
...
begin
  ...
  insert into results values (
    '(c) update: single (u2) can update the label (AC 6: no role check)',
    v_rows_affected = 1,
