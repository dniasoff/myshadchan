 ...
 * be expressed by reading `public.singles` as four different callers — an
 * unrelated household, a grantee through each non-accepted status and then
 * through the accepted/severed lifecycle, a single-role member inside the
 * grantee household, and the accepted grantee — which no mock reproduces:
 * the grant, the RLS policy, current_context_id() and current_member_role()
 * all have to be simultaneously right for (d) to pass and for every negative
 * case to stay closed.
 *
...
const SQL_FILE = path.join(
  ...
  "child_grant_singles_access.sql",
);
