...
/**
 * Runs the `documents` bucket / `add_resume_file` database suite (Story
 * 5.3, AC 2 and AC 4) against the local Supabase stack.
 *
 * The assertions live in documents_storage.sql, because what they check —
 * cross-account RLS on a brand-new storage bucket, deny-by-default for an
 * undefined prefix, and the append-only atomicity of a SECURITY INVOKER
 * function — only exists inside Postgres and cannot be meaningfully
...
// documents_storage.sql wraps its own checks in `begin; ... rollback;` and
...
describe("documents_storage (database)", () => {
