...
/**
 * Runs the child_grants shidduchim-read-across guard against the local
 * Supabase stack.
 *
 * The assertions live in child_grant_shidduchim_access.sql because they can
 * only be expressed by reading `public.shidduchim` as five different callers —
 * an unrelated household, a grantee through a pending and then the accepted
 * status, a single-role member inside the grantee household, and finally the
 * accepted grantee again to pin that shidduch_close_reason() stays NULL for
 * them — which no mock reproduces: the grant, the RLS policy, current_context
 * _id() and current_member_role() all have to be simultaneously right for (c)
 * to pass and for every negative case to stay closed.
 *
...
const SQL_FILE = path.join(
  ...
  "child_grant_shidduchim_access.sql",
);
...
describe("child_grants shidduchim read access (database)", () => {
