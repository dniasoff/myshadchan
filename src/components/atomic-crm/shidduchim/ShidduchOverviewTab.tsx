...
import { ShidduchEducationSection } from "./ShidduchEducationSection";
 ...
 * §2 rule 1), so the `redts` / `shadchanim` / `shidduch_education` fetches
...
export const ShidduchOverviewTab = () => {
  ...
  const { data: schools } = useGetList<ShidduchEducation>(
    "shidduch_education",
    {
      filter: { shidduchim_id: record?.id },
      pagination: { page: 1, perPage: 100 },
