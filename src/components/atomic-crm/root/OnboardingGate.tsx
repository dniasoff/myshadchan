 ...
 *
 * Shows the choice screen whenever the account has no singles AND isn't
 ...
 * stops showing once a single exists or `seedDemo()` flips `isDemo` — no
...
export const OnboardingGate = ({ children }: { children: ReactNode }) => {
  const { total: singlesTotal, isPending: singlesPending } = useGetList<Single>(
    "singles",
    {
  ...
  if (singlesPending || demoPending) {
    return <>{children}</>;
  }

  const hasSingles = (singlesTotal ?? 0) > 0;
  const showOnboarding = !hasSingles && isDemo !== true;
