...
export type Account = {
  ...
  /** When true, photos require an explicit click before a signed URL is
   * requested. The optional shape keeps pre-migration/FakeRest fixtures
   * fail-safe: missing means the product default, false. */
  photo_reveal_on_click?: boolean;
  /**
   * Epic 11 (inbound email capture): the household's own capture address,
   * `citext`, 12 hex chars, auto-generated by
   * `set_account_inbound_email_token_default()` on insert — never
   * client-writable. Always `null` for a `shadchanus`-kind account
   * (`accounts_inbound_email_token_kind_check`, `01_tables.sql`): only a
   * household has an inbox to capture into. The full address is
   * `${inbound_email_token}@myshadchan.space` — see
   * `settings/CaptureSection.tsx`.
   */
  inbound_email_token?: string | null;
  created_at: string;
  demo?: boolean;
  /** FakeRest-only marker mirroring the server's bundle membership helper. */
  demo_bundle_context?: boolean;
} & Pick<RaRecord, "id">;
...
export type MyContext = {
  ...
  role: MemberRole;
  is_active: boolean;
  /** Present only for contexts registered in the official demo bundle. */
  is_demo?: boolean;
};
...
/**
 * Triage state of a captured item: needs confirmation, resolving, resolved,
 * or dismissed. Epic 11 adds `held` — an email-sourced item whose sender is
 * not yet a known member or a `TrustedSender` for the household
 * (`inbox_items_status_check`, `01_tables.sql`). A `held` item is stored
 * exactly like any other — never dropped — but kept out of the working
 * inbox (`inbox/InboxList.tsx`'s `status: 'unresolved'` filter, unchanged)
 * until reviewed in the "Needs review" tab (`inbox/NeedsReviewDialog.tsx`):
 * TRUST releases it (and every other `held` row from the same sender) to
 * `unresolved`; DISCARD moves it to `dismissed`, same as any other item.
 */
export type InboxStatus =
  "unresolved" | "resolving" | "resolved" | "dismissed" | "held";
...
/**
 * One row of `public.trusted_senders` (Epic 11) — an email address a
 * household has explicitly vouched for. The ingest Worker consults this
 * table (via `trusted_senders.email`, `citext`) to decide whether a NEW
 * inbound email is `unresolved` or `held`; it never gates on this table
 * being empty (`classifySender.ts`, `workers/ingest/`). `(account_id,
 * email)` is unique — one trust row per household per address, ever
 * (`trusted_senders_account_id_email_key`, `01_tables.sql`). Written only
 * through `dataProvider.trustSender()` (`providers/supabase/
 * trustedSenders.ts` / `providers/fakerest/internal/trustedSenders.ts`),
 * never a raw `dataProvider.create`.
 */
