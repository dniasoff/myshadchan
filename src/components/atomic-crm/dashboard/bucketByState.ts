...
/**
 * Bucket a single's shidduchim into ordered per-state counts for the pipeline
 ...
 * legend stable regardless of which states the single currently has.
 */
export const bucketByState = (summaries: ShidduchSummary[]): StateBucket[] =>
  PIPELINE_STATES.map((def) => ({
    ...
    label: def.label,
    token: def.token,
    count: summaries.filter((item) => item.pipeline_state === def.value).length,
  }));
