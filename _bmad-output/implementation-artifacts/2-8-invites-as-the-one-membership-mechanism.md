...
### Review Fixes (adversarial review pass, commit `4bb90bb` reviewed)

**Fixed:**

- **Should-fix #1 — the Revoke button was rendered for callers who cannot revoke.** `InvitesSection.tsx` gated the button on `status === "pending"` only, never on the caller's own authority — a `helper`/`single` saw a Revoke button on every pending row and got a server refusal on click, the exact "error after submitting" UX AC-1 designs the send form against (the send form itself was already correctly gated). Added `roleOptions.length > 0` to the button's condition — `roleOptions` is `[]` for any non-invite-capable caller (`invitableRoles()` returns `[]` when `isInviteCapableRole(callerRole)` is false), the same signal already used to hide the form. Added the missing negative test the review flagged: `InvitesSection.test.tsx` now asserts a `helper` sees no Revoke button even when the list contains a genuinely `pending` invite.
- **Should-fix #2 — error toasts hard-coded English; the two `invites_*_error` i18n keys were unreachable dead copy.** Both `handleSend`/`handleRevoke` passed `error.message` (raw Postgres text) as the notify **key**; since `i18nProvider` is built with `allowMissing: true`, a raw message never matches a key and always falls back to the inline `_` string — so the French `invites_send_error`/`invites_revoke_error` catalogue entries (added in Task 5) could never render, and French users always got the English fallback text regardless of locale. Took the review's stated minimal fix (`PersonasSection`'s fuller per-message mapping was the alternative offered, but `create_invite()`/`revoke_invite()`'s five distinct raise messages aren't user-actionable in a way that justifies five more copy keys × two locales — the existing generic keys already exist and are already translated in both catalogues): both handlers now always `notify()` with the real translated key (`crm.settings.invites_send_error` / `invites_revoke_error`), never with `error.message`. This makes the two catalogue keys reachable for the first time.
- **Should-fix #3 — no e2e spec for a new form/interaction surface.** Added `e2e/invite-sending.spec.ts`: signs in a `parent_admin`, sends an invite from Settings (AC-1), asserts the shareable link renders inline (AC-2), asserts the new invite lists as `Pending` with a Revoke action (AC-3/AC-4), clicks Revoke, and asserts the status flips to `Revoked` and the action disappears. Also fixed the stale comment in `e2e/invite-acceptance.spec.ts:8` ("there is no in-app way to SEND an invite yet (that is story 2.8)") to point at the new spec instead.
- **Should-fix #4 — revoke/accept lost-update race.** `revoke_invite()` read the invite with a plain `SELECT`, checked `status = 'pending'`, then ran an unconditional `UPDATE` — under READ COMMITTED, a concurrent `accept_invite()` could bind a membership from the same invite between the read and the write, and both could commit (the admin believes they cancelled; the invitee is inside). Took the review's stated alternative to explicit row locking: both functions now fold their status check directly into the `UPDATE`'s `WHERE` clause (`... where id = ... and status = 'pending'`) and raise when `not found` (zero rows affected) rather than checking a separately-read snapshot. Postgres's EvalPlanQual mechanism re-evaluates that `WHERE` predicate against the latest committed row once any lock a concurrent writer held is released, so whichever of `revoke_invite()`/`accept_invite()` commits first wins the row and the other correctly sees it already transitioned and raises, instead of clobbering it. `accept_invite()` was additionally reordered — the atomic `UPDATE` (claiming the invite) now runs BEFORE the `account_members` insert, so a losing `accept_invite()` never creates a membership at all. All 52 existing `invites.sql` checks (none of which assert exact error-message text for these two paths) still pass unmodified; no new SQL check was added for the race itself, since exercising true statement-level concurrency isn't practical inside the suite's single-connection `psql -f` harness — flagged here as an accepted gap rather than silently skipped.
- **Note #5 — greenfield orphan `public.get_user_id_by_email(text)`.** Its only caller (`inviteUser()`) was deleted by this story's own Task 4, leaving it live with no reader anywhere in the repo (re-verified: zero hits outside its own declaration/grant and old, never-edited migrations). Dropped the function and its two grant lines from `02_functions.sql`/`06_grants.sql`.

**Rejected (evidence below):**

- **Note #6 — the invite list (every invitee's email) is visible to every household member, not just invite-capable roles.** The review itself hedges this as "Fix (if intended)." Rejected as out of this pass's scope: AC-4 specifies `useGetList("invites", …)` renders "each invite's email, role, status" with no role qualifier, and the underlying visibility is Story 2.7's own "Invites readable within active account" SELECT policy (any active member) — a deliberate, already-shipped RLS decision this story's UI merely surfaces, not a defect this story introduced. Gating the whole section by role (rather than just the Revoke button, which finding #1 already does) would change AC-4's specified behavior beyond what the review asks for and would need its own sign-off, not a silent scope change here.
- **Note #7 — an expired-but-stored-`pending` invite is unrevokable through the UI.** The review itself flags this as harmless and "consistent with the story's 'expiry is read-time' note," asking only that it be a known decision rather than an oversight — which this response records. No code change made.

**Toolchain re-verified after fixes:** `npm run typecheck` clean, `npm run lint` clean, `npm run prettier` clean, `npm run test` 826/826 (68 files, +1 for the new negative test), `npm run test:unit:db` 325/325 (6 suites, `invites.sql` still 52 checks — unchanged in count, all passing against the reordered functions), `npm run build` clean, `node scripts/check-retired-names.mjs` OK, `node scripts/check-suppressions.mjs` OK, `make registry-gen` no diff. `DBUS_SESSION_BUS_ADDRESS=/dev/null npx supabase db diff --local` → "No schema changes found" after applying migration `20260728074040_invite_race_and_cleanup.sql` (hand-checked: only the `get_user_id_by_email` drop plus the two rewritten function bodies — no GRANT/REVOKE churn since dropping a function drops its own grants, no `storage.objects` churn).

### File List

**Review-fix pass (this section) additions/changes, on top of the original File List below:**
- `src/components/atomic-crm/settings/InvitesSection.tsx` — gated the Revoke button on `roleOptions.length > 0` (Should-fix #1); both error handlers now always `notify()` with the translated key, never `error.message` (Should-fix #2).
- `src/components/atomic-crm/settings/InvitesSection.test.tsx` — added the missing "helper sees no Revoke button" negative test (Should-fix #1).
- `e2e/invite-sending.spec.ts` — new spec: send → link visible → revoke → status flips (Should-fix #3).
- `e2e/invite-acceptance.spec.ts` — fixed the stale "no in-app way to SEND an invite yet" comment (Should-fix #3).
- `supabase/schemas/02_functions.sql` — `revoke_invite()`/`accept_invite()` now fold their status guard into the claiming `UPDATE`'s `WHERE` clause instead of checking a separately-read snapshot (Should-fix #4); dropped the orphaned `get_user_id_by_email()` (Note #5).
- `supabase/schemas/06_grants.sql` — dropped `get_user_id_by_email()`'s grant lines (Note #5).
- `supabase/migrations/20260728074040_invite_race_and_cleanup.sql` — generated migration for the above.
