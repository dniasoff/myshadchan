...
- `node scripts/check-route-convention.mjs` / `check-tailwind-arbitrary-var.mjs` → OK. `check-retired-names.mjs` / `check-suppressions.mjs` still report **pre-existing, unrelated** failures (`adminRouteBuilders.tsx`'s `1.3-children-camelcase` matches; the atomic-crm `eslint-disable` budget already at 4/3) — confirmed via `git diff --stat HEAD -- <file>` showing zero diff on every implicated file; not touched by this story.
