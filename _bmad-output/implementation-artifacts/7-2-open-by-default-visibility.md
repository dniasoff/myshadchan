...
- [ ] **Task 2 — Grants** (AC: 5)
  - [ ] `supabase/schemas/06_grants.sql:835`: extend the existing column list to
  ...
  - [ ] Do **not** invent new role-gating. The shipped `accounts` policies already handle it
        (Dev Notes "Who may change the default posture"), and `name`/`transparency_level`
        set the precedent for this column list.

- [ ] **Task 3 — `create_thread()` reads the account default** (AC: 3, 4)
  - [ ] `supabase/schemas/02_functions.sql`: change the one expression in `create_thread()`
  ...
  - [ ] Keep 7.1's `p_visibility in ('open','private')` validation ahead of the coalesce, so
        an invalid explicit argument still raises rather than falling through to the
        default.
  - [ ] Hand-check the generated migration actually contains the `CREATE OR REPLACE
...
- [ ] **Task 4 — Types and fixtures** (AC: 1, 5)
  - [ ] `src/components/atomic-crm/types.ts`: add `default_thread_visibility:
        ...
        same file).
  - [ ] **Adding a required field to `Account` breaks every site that constructs one.** Six
        ...
        complete.
  - [ ] `providers/fakerest/dataProvider.ts`: the FakeRest `createThread` emulation (7.1)
        resolves the default from `db.accounts` the same way, so AC-3/AC-4 hold in the demo
        build too (AD-10).

- [ ] **Task 5 — Settings UI** (AC: 5)
  - [ ] New file `src/components/atomic-crm/settings/CommunicationSection.tsx`. Do **not**
        ...
        by Task 2).
  - [ ] Gate rendering on `useViewerRole()` (`entity360/useViewerRole.ts`) — hide the
  ...
  - [ ] Wire it into **both** section lists: `settings/SettingsPage.tsx:60-73` and
  ...
  - [ ] All copy through the `i18nProvider` under `crm.settings.communication.*`, added to
...
- [ ] **Task 6 — Tests** (AC: 6)
  - [ ] Extend `supabase/tests/threads_entity.sql` (created by 7.1) — do not create a second
  ...
  - [ ] Vitest (browser mode, `vitest-browser-react` + `TestMemoryRouter`; RTL is not a
