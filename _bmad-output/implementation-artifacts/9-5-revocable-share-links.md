...
- [x] **Task 1 — Schema: `share_links` and `share_access_log`** (AC: 1, 2, 9, 10)
  - [x] Append to `supabase/schemas/01_tables.sql`:
        ```sql
  ...
  - [x] `single_id` carries the domain's standard composite FK, exactly as
  ...
  - [x] `resource` on `share_access_log` records **what** was accessed on that request (e.g.
        ...
        file download.
  - [x] Indexes: `create index share_links_account_id_idx on public.share_links using btree
  ...
  - [x] Trigger for `token`, mirroring `set_child_portal_token_defaults()` exactly (the deleted
  ...
  - [x] A second, `BEFORE UPDATE` trigger makes revocation one-way (AC-6):
        ```sql
...
- [x] **Task 2 — RLS: manager-side management, zero `anon` reach** (AC: 9, 10)
  - [x] `alter table public.share_links enable row level security;` `force row level security;`
        (AD-1 requires `FORCE` on every table without exception, including one whose "real"
        readers are outside Postgres entirely).
  - [x] `"Share links manager scoped"` — one `for all to authenticated` policy whose `using`
  ...
  - [x] `alter table public.share_access_log enable row level security;` `force row level
  ...
  - [x] `revoke all on table public.share_links, public.share_access_log from anon;` — no
...
- [x] **Task 3 — Generate and hand-check the migration** (AC: all)
  - [x] `DBUS_SESSION_BUS_ADDRESS=/dev/null npx supabase db diff --local -f add_share_links`
  - [x] Hand-check: confirm `FORCE ROW LEVEL SECURITY` on both tables, confirm no `anon` grant
  ...
  - [x] Run `db diff --local` a second time, scoped to `resumes` — it must be **empty**. This is
...
- [x] **Task 4 — The `share/` Worker: fill in the scaffold, on Supabase Storage** (AC: 3, 4, 5,
      6, 7, 10, 13)
  - [x] **Drop the R2 binding.** `workers/share/index.ts` currently declares `ShareEnv extends
  ...
  - [x] Extend the same `Hono` app (do not create a parallel one). Add:
  ...
  - [x] Use `forAccount(shareLink.account_id, env)` (existing `workers/shared/forAccount.ts`) for
  ...
  - [x] `workers/share/index.test.ts` already tests `/health` — extend it, do not replace it.
  ...
  - [x] **Amend the architecture spine in the same diff** (do not leave the written spine
...
- [x] **Task 5 — Epic 5's resume shape (confirmed satisfied — read this before writing Worker
      queries anyway)** (AC: 3)
  - [x] **Verified, not assumed.** `public.resumes` already has a single's own addressable
  ...
  - [x] The photo source is `resume_photos`, filtered by `resume_id` (the resume row found
...
- [x] **Task 6 — Provider and components** (AC: 1, 6, 8)
  - [x] `providers/supabase/dataProvider.ts`: plain `dataProvider.create("share_links", { data:
        ...
        shape.
  - [x] `providers/fakerest/`: add `share_links` and `share_access_log` base resources; since
  ...
  - [x] New directory `src/components/atomic-crm/sharing/` — named exactly as the architecture
  ...
  - [x] **i18n, two different seams in one story.** `CreateShareLinkDialog.tsx` and
  ...
  - [x] `src/App.tsx`: add the `isShareUrl(window.location)` check before `<LandingGate>`,
  ...
  - [x] Where the sharer reaches this from: Settings, next to Story 9.2's
  ...
  - [x] **`registry.json`** — this story adds the whole `sharing/` directory; regenerate with
        `make registry-gen` (or the pre-commit hook) and declare the file as touched.
  - [x] **`share` rejoins the `deploy-workers` matrix.** `.github/workflows/deploy.yml:275`
...
- [x] **Task 7 — Byte cleanup for `resumes` and `resume_photos`** (AC: 11, 12)
  - [x] `providers/supabase/resumes.ts`: add `removeResumeFileObjects(paths: string[]):
  ...
  - [x] `providers/supabase/resumePhotos.ts`: add `removeResumePhotoObjects(paths: string[]):
        Promise<void>`, same shape, for `resume_photos.path` values.
  - [x] New `providers/supabase/resumeStorageCleanup.ts` (own file, following `entityFiles.ts`'s
  ...
  - [x] `dataProvider.ts`: splice `...buildResumeStorageCleanupCallbacks()` into
...
- [x] **Task 8 — Tests** (AC: all)
  - [x] `resumeStorageCleanup.test.ts` — deleting a `singles` row (and separately a `shidduchim`
  ...
  - [x] `supabase/tests/share_links.sql` + `.test.ts` — new database suite, same harness as
  ...
  - [x] `workers/share/index.test.ts` — the full list under Task 4's last bullet.
  - [x] Frontend component tests for `CreateShareLinkDialog`, `ShareLinkList` (revoke action,
  ...
  - [x] `make typecheck && npm run lint && make test && npm run test:unit:db` (the SQL suite) —
...
- `DBUS_SESSION_BUS_ADDRESS=/dev/null npx supabase db diff --local` — clean on both runs (before
  and after applying `20260803032639_add_share_links.sql` and, later,
  `20260803040047_share_links_single_account_check.sql`); `resumes`' column order unaffected
  (verified directly against `pg_attribute` — `single_id` stays at the physical tail).
- `make check-migration-safety STACK_ID=1` — **red, pre-existing, not this story's fault.** Fails
  at step `[2/4] seeding production-shaped rows`, in `supabase/tests/migration-data-safety/
  fixture.sql:533`, with `null value in column "proposed_by_account_id" of relation "connections"
  violates not-null constraint`. Proven pre-existing per the "if a gate is red, prove it" rule:
  the failure occurs during the fixture's own seed step, which runs AFTER the guard resets to the
  **pre-9.5 baseline** (`20260802191849`, before this story's migration is even applied) — so the
  failure is reproducible with zero of this story's SQL in play. `connections.proposed_by_
  account_id` was made `not null` by Story 8.2 (`7e0dadc`); the fixture's own `connections` seed
  block (last touched by Story 9.3's review-fix commit `fa01de2`) was never updated to supply it.
  This story's own migrations are purely additive (two new tables, no `drop`/`narrow`/`rename` of
  any existing column), so there is no data-loss risk for this gate to actually be checking here —
  but the gate itself cannot run to completion regardless, for a reason entirely outside
  `share_links`/`share_access_log`. Not fixed here: `supabase/tests/migration-data-safety/
  fixture.sql` is outside this story's declared file set and touching `connections` risks
  colliding with other concurrent work on this branch.
- `npx vitest --project db run supabase/tests/share_links.test.ts` — 50/50 green (also re-run as
  part of the full `db` project: 34 files / 1324 tests green).
- `npx vitest --project workers run` — 45/45 green (includes `workers/share/index.test.ts`, 20
  tests).
- `npx vitest --project app run` — 213 files / 1658 tests green.
- `make test STACK_ID=1` (full suite against the isolated stack, migrations applied fresh) —
  276 files / 3333 tests green; stack stopped and released afterward.
- `make typecheck`, `npm run lint`, `npx prettier --check` (on this story's changed files —
  `.github/workflows/deploy.yml` and the two new `.sql` suites are pre-existing-unformatted /
  parser-less respectively, not this story's regression), `make build`, `make registry-gen`, and
  all four CI guards (`check-suppressions.mjs`, `check-retired-names.mjs`,
  `check-route-convention.mjs`, `check-tailwind-arbitrary-var.mjs`) — all green. The retired-name
  guard initially failed on six comments that named the deleted token-portal's own identifiers
  (`ChildPortalPage`, `get_child_portal()`, `set_child_portal_token_defaults()`,
...
- **Storage ruling applied as specified**: `workers/share/index.ts` has no R2 binding (`ShareEnv`
  is a plain `BaseEnv`); `workers/share/wrangler.toml`'s `[[r2_buckets]]` block is gone; the Worker
  streams `GET /r/:token` (profile + opaque file manifest) and `GET /r/:token/file/:fileKey`
  (proxied byte stream) via `@supabase/supabase-js` against the `documents` bucket, using a raw
  service-role client only for the initial token resolution and the Storage read — every
  tenant-table read goes through `forAccount()`.
- **AC-13's opaque `fileKey`**: the manifest (`resume-0`, `resume-1`, ..., `photo`) is rebuilt
  fresh on every request from the resolved `share_links` row; a forged value, a path-traversal
  string, and a real key that only exists on a *different* link's manifest all produce the
  identical 404 with the mocked `.storage.from("documents")` never invoked (asserted directly,
  not merely via response shape) — three distinct negative tests in `workers/share/index.test.ts`.
- **Schema hardening beyond the ticket's literal SQL, found by this story's own SQL suite**: the
  "Share links manager scoped" policy's literal Task 2 text let a `parent_admin` INSERT naming
  *another* account's `single_id` — refused only by the composite `share_links_single_id_fkey` FK
  (23503), never by RLS (42501), because the policy's `parent_admin` branch never checked that
  `single_id` actually belongs to the caller's own account. Added `single_id in (select s.id from
  singles where account_id = current_context_id())` to both `using` and `with check`, mirroring
  `listings`' own "Single listings insert/update" policies exactly. Migration
  `20260803040047_share_links_single_account_check.sql` carries this as its own drop+recreate;
  `db diff --local` confirmed clean after.
- **Epic 5 dependency verified, not assumed**: `resumes.single_id` / `resumes_owner_check` /
  `resumes_single_id_key` read directly from `01_tables.sql` before writing the Worker's query
  shape; no fallback branch built (matches the story's own Task 5 instruction).
- **AC-11/AC-12 byte cleanup**: `removeResumeFileObjects`/`removeResumePhotoObjects` added to
  `resumes.ts`/`resumePhotos.ts` (mirroring `entityFiles.ts` exactly, including a `try/catch`
  around the storage call itself — the AC-12 test specifically wants a *rejected* `.remove()`
  promise, not merely a resolved `{ error }` tuple, to never propagate); `resumeStorageCleanup.ts`
  (new) builds the two `beforeDelete` callbacks for `singles`/`shidduchim`, spliced into
  `lifeCycleCallbacks` next to the existing `entityFilesCleanupCallbacks`.
- **`ShareSingleDialog.tsx`** (new, in `sharing/`, not separately named by the story's own file
  list) composes `ShareLinkList` + `CreateShareLinkDialog` behind one "Share" button per row in
  `settings/SingleListingSection.tsx`, gated by the same `canPublish()` authority check the
  Publish action already uses (RLS is the real boundary either way).
- **`vite.config.ts`** gained one `define` entry, `VITE_SHARE_WORKER_URL` — without it,
  `sharing/shareClient.ts`'s `fetch()` call has no Worker origin to reach in a production build
  (the exact landmine already named for `VITE_VAPID_PUBLIC_KEY`/a hypothetical
  `VITE_AI_WORKER_URL`).
- **`.github/workflows/deploy.yml`**: `share` rejoins the `deploy-workers` matrix
  (`[ingest, parse, match, ai, cron, share]`); the withholding comment rewritten to record that
  the R2 blocker is resolved by dropping the binding, not by enabling R2.
- **`ARCHITECTURE-SPINE.md`**: every R2 mention tied to this decision updated for internal
  consistency, not only the four locations the story names explicitly (AD-9, AD-15, the Stack
  table's Media row, the Structural Seed diagram) — also the "Layer → home" summary line, the
...
- **Things this story did NOT do, on purpose**: no FakeRest emulation of the Worker's proxy stream
  ...
  no rate limiting on the public `/r/:token` path (AD-17, explicitly named in this story's own Dev
  Notes as unowned by any story in Epics 1–11, flagged rather than silently absorbed here); no
  widening of a plain `single`'s own visibility/control over links naming them (Dev Notes "Why
  share links are manager-scoped, not household-scoped" flags this as a future story's own
  decision, not a quiet policy tweak here).

### File List

**Schema**
- `supabase/schemas/01_tables.sql` — `share_links`, `share_access_log` tables, composite FKs,
  indexes.
- `supabase/schemas/02_functions.sql` — `set_share_link_token_defaults()`,
  `enforce_share_link_revoke_once()`.
- `supabase/schemas/04_triggers.sql` — the two triggers on `share_links`.
- `supabase/schemas/05_policies.sql` — FORCE RLS + `"Share links manager scoped"` +
  `"Share access log readable by link owner"`.
- `supabase/schemas/06_grants.sql` — anon-zero grants, `authenticated`'s `select, insert` +
  column-level `update (revoked_at)` on `share_links`, `select`-only on `share_access_log`,
  sequence grants.
- `supabase/migrations/20260803032639_add_share_links.sql` — hand-verified (FORCE RLS, grants,
  sequences added by hand per AGENTS.md).
- `supabase/migrations/20260803040047_share_links_single_account_check.sql` — the policy-hardening
  follow-up.
- `supabase/tests/share_links.sql` (new), `supabase/tests/share_links.test.ts` (new) — 50 checks.
...
- `src/components/atomic-crm/sharing/shareClient.ts` / `.test.ts`
- `src/components/atomic-crm/sharing/sharedProfileTranslate.ts`
- `src/components/atomic-crm/sharing/SharedProfilePage.tsx` / `.test.tsx`
- `src/components/atomic-crm/sharing/CreateShareLinkDialog.tsx` / `.test.tsx`
- `src/components/atomic-crm/sharing/ShareLinkList.tsx` / `.test.tsx`
- `src/components/atomic-crm/sharing/ShareSingleDialog.tsx` (new, composition wrapper — see
  Completion Notes)

**Frontend — wiring**
- `src/App.tsx` / `src/App.test.tsx` — `isShareUrl` pre-CRM branch.
- `src/components/atomic-crm/settings/SingleListingSection.tsx` / `.test.tsx` — the "Share"
  action per row.
- `src/components/atomic-crm/types.ts` — `ShareLink`, `ShareAccessLog`, `ShareLinkExpiryDays`.
- `vite.config.ts` — `VITE_SHARE_WORKER_URL` in the production `define` block.

**Provider — Supabase**
- `src/components/atomic-crm/providers/supabase/dataProvider.ts` — `revokeShareLink`,
  `getShareAccessLog` custom methods; `resumeStorageCleanupCallbacks` spliced into
  `lifeCycleCallbacks`.
...
- `src/components/atomic-crm/providers/fakerest/dataProvider.ts` — `share_links` create-time
  token-stamping, `revokeShareLink`, `getShareAccessLog`.
- `src/components/atomic-crm/providers/fakerest/internal/shareLinks.ts` (new).
- `src/components/atomic-crm/providers/fakerest/dataGenerator/index.ts` / `types.ts` —
  `share_links`/`share_access_log` seeded empty.
- `src/components/atomic-crm/providers/fakerest/dataProvider.shareLinks.test.ts` (new).

**i18n / registry / deploy / docs**
- `src/components/atomic-crm/providers/commons/englishCrmMessages.ts` /
  `frenchCrmMessages.ts` — the `sharing.*` keys and `single_listing.share_button`.
- `registry.json` — regenerated (`make registry-gen`).
- `.github/workflows/deploy.yml` — `share` rejoins the `deploy-workers` matrix; withholding
  comment corrected.
- `_bmad-output/planning-artifacts/architecture/architecture-myshadchan-2026-07-21/
  ARCHITECTURE-SPINE.md` — AD-9, AD-15, Stack table, Structural Seed diagram, Capability map,
  Deferred section: R2 → Supabase Storage.
