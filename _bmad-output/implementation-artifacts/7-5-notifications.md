...
(`workers/cron/index.ts`'s `scheduled()` handler is a one-line `console.warn` stub;
there is no Resend integration, no push-subscription table, and no VAPID key handling
anywhere in the repo). This story is the first to build real outbound delivery, and
...
- [ ] **Task 1 — Schema: read state and the delivery queue** (AC: 1, 2, 3, 4, 5)
  ...
  - [ ] Dual-axis scope on `message_notifications` (account_id/connection_id, XOR
        ...
        `(connection_id, message_id) references messages(connection_id, id)`.
...
- [ ] **Task 2 — Fan-out trigger** (AC: 2, 3, 4, 5)
  - [ ] `supabase/schemas/02_functions.sql`: `fan_out_message_notifications()`
        ...
        `thread_participants` row on `new.thread_id` where `member_id <>
        new.sender_member_id`: insert one `message_notifications` row with
        `channel = 'email'` unconditionally (AC-2); insert a second row with
        `channel = 'push'` **only if** `exists (select 1 from push_subscriptions where
        member_id = <that participant>)` (AC-3). Copy `account_id`/`connection_id`
        from `new` (whichever is non-null) onto every inserted row.
...
- [ ] **Task 3 — `mark_thread_read()` and RLS/grants** (AC: 1, 7)
  - [ ] `public.mark_thread_read(p_thread_id bigint) returns public.thread_participants`
        ...
        last_read_at = now() from account_members am where am.id = tp.member_id and
        am.user_id = auth.uid() and am.status = 'active' and tp.thread_id =
        p_thread_id returning tp.*`. The join to `account_members` on `auth.uid()` is
        the entire authorization check — it can only ever touch a participant row
        belonging to the caller's own membership, by construction, with no separate
        context check needed (a caller with no matching `thread_participants` row
  ...
  - [ ] `message_notifications` and `push_subscriptions`: `enable row level security`
        ...
        push_subscriptions.member_id and am.user_id = auth.uid()))` — a user manages
        ...
        for push isn't a tenant-data read).
  - [ ] `06_grants.sql`: revoke `anon` on both new tables; grant `select, insert,
        ...
        delete+insert, simpler); grant `all` on both to `service_role`; grant execute
        on `mark_thread_read`/`fan_out_message_notifications` appropriately (the
        trigger function needs no direct grant, only the table triggers wired to it).
...
- [ ] **Task 4 — Outbound email via Resend** (AC: 2, 6)
  - [ ] New file `workers/shared/resend.ts`: a minimal `sendEmail({ to, subject,
        html })` wrapper around `POST https://api.resend.com/emails` using a
        `RESEND_API_KEY` secret (Wrangler secret, never in client bundle — matches the
  ...
  - [ ] New file `workers/cron/notifyMessages.ts`: the sweep. Selects
        `message_notifications` where `status = 'pending'`, **claims** them in the
        same query (`update … set status = 'sending' where id in (select id … for
        update skip locked) returning *` — `for update skip locked` is what makes
        AC-6's "no duplicate send under overlap" true even across two concurrent
        sweeps, not just one). For `channel = 'email'`, resolve the recipient's email
        (join `account_members.user_id` → `auth.users.email` via the service-role
        client) and the thread's context for a subject line, call `sendEmail()`, then
        set `status = 'sent', sent_at = now()` or `status = 'failed', error = …` on
        failure. Uses the service-role Supabase client directly (**not**
        `forAccount()`, which the notification sweep can't use — this is
        cross-tenant system work, the same exemption `workers/cron/index.ts`'s
        reminder sweep already relies on for the identical reason).
  - [ ] `workers/cron/index.ts`: replace the `console.warn` stub in `scheduled()` with
        ...
        Add/confirm a `[triggers]` cron schedule in `workers/cron/wrangler.toml`
        frequent enough for a "you have a new message" email to feel timely (every 1
        minute is a reasonable starting point; document the choice in a comment, this
        is a product/cost tradeoff the story owner is making explicitly, not leaving
        open).
...
- [ ] **Task 8 — Types and provider** (AC: 1, 2, 3)
  - [ ] `types.ts`: `MessageNotificationChannel = "email" | "push"` (mirrors
        `TaskDeliveryChannel` exactly in shape — **do not** rename or refactor
        `TaskDeliveryChannel` itself to be shared; that is a cross-epic cleanup for
        the epic owner to schedule deliberately, not a silent side effect here — see
        Dev Notes), `PushSubscription` type, `last_read_at?: string | null;` on
        `ThreadParticipant`.
...
- [ ] **Task 9 — Tests** (AC: 7)
  - [ ] New `supabase/tests/message_notifications.sql` + `.test.ts` (separate file
        ...
        (AC-5); `mark_thread_read()` only touches the caller's own row; and the
        negative test — a second user's client reads/updates/deletes zero rows of
        another user's `push_subscriptions`.
  - [ ] `workers/cron/notifyMessages.test.ts` / `webPush.test.ts` / `resend.test.ts`:
...
`reminders/reminderEntity.ts` already defines `TaskDeliveryChannel = "in_app" | "email"
| "push"` for the (separate, old-numbering) reminders feature. This story's
...
touch a file — `reminders/reminderEntity.ts` — that belongs to a different, already-
built feature, with its own review surface).
...
than double-sending. This needs a service-role Postgres connection (RPC or a direct
`pg` connection from the Worker), since `message_notifications` has no `authenticated`
policy at all.
...
- `workers/cron/index.ts` (current stub, replaced by Task 4).
- `src/components/atomic-crm/reminders/reminderEntity.ts:1-26`
  (`TaskDeliveryChannel` — the vocabulary this story mirrors but does not refactor).
