...
### Review Findings — Fix Pass (2026-07-27)

Addressed the adversarial review of commit `4901d6a`. Findings and disposition:

1. **Fixed (should-fix) — `supabase/functions/mcp/index.ts` (`query` and `mutate` tool descriptions).** After this story's rename, `member_id` is ambiguous between `tasks.member_id` (auto-set, points at `public.members`) and `singles.member_id` (settable, points at `public.account_members`). Reworded both sentences to scope the guidance per-table instead of treating `member_id` as a single concept: the `query` tool description now says to filter `tasks` on `member_id = auth.uid()` but warns that on other tables (e.g. `singles`) `member_id` references `account_members` and must not be compared to `auth.uid()`; the `mutate` tool description now says never to set `member_id` on `tasks` (trigger-owned) but that other tables' `member_id` (e.g. `singles`) may be set normally. No test asserts on the literal prompt text (`supabase/functions/mcp/*.test.ts` only covers `validateSql.ts`), so this was a safe rewording. Re-ran the full AC-14 gate afterward — still zero hits.
2. **No code change — AC 7 / Task 6's `useGetMemberName.ts` file-list entry.** Confirmed again: `src/components/atomic-crm/sales/useGetSalesName.ts` was already deleted in `d66119c` (story 1.1's own review-findings commit) because all 10 of its importers were fossil files deleted in that same pass. Recreating a zero-caller hook here would itself be a fresh NFR-14 violation. This is a story-file wording issue, not a code defect — leaving for the epic owner to amend AC 7 / Task 6's target file list, per the review's own recommended fix.
3. **Flagged, not hand-edited — repo-root `MEMORY.md` lines 7 and 9.** Confirmed still stale post-rename (`"...and sales (team members)"`, `"Sales users sync with Supabase auth.users..."`). Per Task 13's explicit instruction ("flag, do not hand-edit") this file is the `documentator` agent's to update, so it was left untouched. Flagging here for `documentator` / epic owner: replace "sales (team members)" → "members (team members)" and "Sales users sync" → "Members sync" at `MEMORY.md:7,9`.
4. **Recorded evidence — Task 14's manual smoke subtask.** Ran the three UI-equivalent paths end-to-end against the local stack (`API_URL=http://127.0.0.1:54321`) using a disposable throwaway user, cleaned up afterward with zero residue (verified: pre-existing `members` row count unchanged at 3):
   - Created an auth user via the admin API → `handle_new_user()` inserted `public.members` row `id=753` (visible via `GET /rest/v1/members?user_id=eq.<uid>` → 200, matching the `/members` list read path).
   - Signed in (password grant) and `PATCH functions/v1/users` with body `{"member_id":753,...}` → 200, updated `first_name`/`last_name` — proves the client's `member_id` body field and the edge function's field name agree end-to-end (the `/settings` profile-edit path).
   - Granted the throwaway user an isolated account/shadchan (service-role inserts, additive only) and `POST /rest/v1/tasks` with no `member_id` in the body, `target_type=shadchan` → 201 with `member_id:753` in the response — proves `set_member_id_default()` auto-populates from `auth.uid()` on the create-reminder path.
   - Cleaned up: deleted the task, shadchan, account_members row, account, members row and auth user; confirmed zero rows remain under the throwaway email/account/task ids and the pre-existing `members` count is still 3.
5. **Fixed (note) — two mechanically-renamed comments.** `supabase/schemas/02_functions.sql`: "the legacy `members` row" → "the `members` profile row" (the fossil `sales` concept is gone; `members` is now canonical, so "legacy" was backwards). `supabase/schemas/01_tables.sql`: "Private schema (used by members policies migration)" → "Private schema (created by a historical RLS-cleanup migration; currently unused, kept to avoid a spurious drop)" — the actual historical migration is `supabase/migrations/20241104153231_sales_policies.sql` (append-only, correctly left unrenamed), so a comment claiming a "members policies migration" exists was factually wrong; naming the real migration file instead would reintroduce a live `sale` hit inside `supabase/schemas/`, which the AC-14 gate sweeps, so the comment was reworded to describe the schema's purpose without naming either retired or real migration filenames. Re-ran `db diff --local` after both comment edits — "No schema changes found" (SQL line comments aren't part of the compiled schema, so no drift).
6. **Fixed (note) — `dataProvider.ts` mislabeled `console.error`.** `memberUpdate`'s failure branch logged `"memberCreate.error"` (carried verbatim from the pre-rename `"salesCreate.error"` mislabel inside `salesUpdate`, per Task 7's literal wording). Changed to `"memberUpdate.error"`. No test or code depends on the literal string (verified via search).
7. **Rejected (as scoped) — `updatePassword(_id: Identifier)` ignoring its argument.** Confirmed the parameter only survives because `providers/fakerest/dataProvider.ts`'s `updatePassword` still consumes `id` for its own edge-simulation logic; the Supabase provider's edge function resolves the user from the JWT and never reads the body. Per the review's own recommendation, this is a cross-provider divergence to resolve in story 1.6 (or whenever the two providers' `updatePassword` signatures are reconciled), not something to diverge the providers over inside this rename story. No change made.
8. **No action — deploy coupling (frontend/migration/`users` edge function must ship together).** Not a code defect; reconfirmed in the smoke test above that the client's `member_id` body field and the deployed edge function's field name agree locally. Flagging for the deploy round: production is still pre-Epic-1, so this story's frontend, migration and `users` edge function must land in the same deploy to avoid a 404 on profile saves.

Re-ran the full validation suite after all fixes: `npm run typecheck` clean, `npm run test:unit:app` 516/52 green, `npm run test:unit:functions` 76/3 green, `npm run test:unit:db` 143/4 green, `npx prettier --config ./.prettierrc.json --check` clean on the touched files, AC-14 gate sweep still zero hits, `db diff --local` still reports no drift.
