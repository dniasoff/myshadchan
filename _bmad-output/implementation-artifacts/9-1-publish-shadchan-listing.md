...
- 2026-08-03 — Review fixes (F1–F7): `authenticated` no longer holds `TRUNCATE`/`REFERENCES`/`TRIGGER` on `listings` (F1 — TRUNCATE bypasses RLS); the AC-8 grant test now asserts the exact ACL rather than only "at least these four verbs" (F2); the household-negative test in `ShadchanListingSection.test.tsx` no longer accepts a false-pass from the panel's own loading skeleton (F3); the `listings_id_seq` grants `06_grants.sql` already declared now actually reach the database (F4); `listings` gained a `to_regclass`-guarded seed + capture block in the migration-data-safety fixture (F5); `anon`'s SELECT on `listings` is now an enumerated column grant that excludes `account_id`/`single_id`/`published_by_member_id` (F6); un-publishing a field (Area off, republish) is now pinned by its own test (F7). New migration `20260802224935_listings_review_fixes.sql`. Status stays → review for re-review.
...
- Review-fix pass: `DBUS_SESSION_BUS_ADDRESS=/dev/null npx supabase db diff --workdir .supabase-e2e-1 -f listings_review_fixes` (STACK_ID=1) — `migra` caught F1's four table-level revokes on its own (`anon`'s old blanket `select`, `authenticated`'s `references`/`trigger`/`truncate`) but emitted nothing for F6's new column-level `anon` grant or F4's sequence grants, confirming the repo's own "`db diff` never re-emits grants" rule for both cases; both hand-added to `20260802224935_listings_review_fixes.sql`. Applied via `supabase migration up --workdir .supabase-e2e-1`, then re-ran `db diff` against a from-scratch rebuild of the stack — "No schema changes found".
...
- Review-fix pass re-verified `check-migration-safety`'s pre-existing failure independently (`git log -- fixture.sql` last touched in `2504e29`, before Story 8.5's `96a548a` added the NOT NULL columns; `git diff` shows it byte-identical across `3032e14`(origin/main)/`ec81675`/`e824da6`) and found the fixture's `connections` seed block is missing **two** NOT NULL columns Story 8.5 added, not one: `proposed_by_account_id` (already reported) and `household_account_name` (not previously reported). Neither is fixed here — out of this story's declared File List and not caused by it — but is called out for whoever picks up the fixture gap next, since a single-column patch alone will still fail. F5's own new `listings` seed block was validated end-to-end against a scratch copy of the fixture with both columns patched locally (not committed): the row inserts, `migration_guard.capture('listings')` captures it, and the script completes to `rollback` cleanly.
- Review finding F8 (minor, not fixed): `anon` still holds `REFERENCES`/`TRIGGER`/`TRUNCATE` on `public.members` and `public.configuration`. Agreed as real and as making AC-8's "anon holds select on listings and nothing else" line about the schema as a whole still not literally true — but it is pre-existing (predates `listings` entirely, on tables this story never touches), not a `listings`-specific leak (TRUNCATE bypasses RLS but leaks no row data on its own, unlike a `select`-shaped hole), and is the exact shape of the repo-wide FORCE ROW LEVEL SECURITY / TRUNCATE-hardening retrofit `epics.md`'s Unowned-work item S2 already tracks for ~33 tables. Folding two more tables from that retrofit into this story's diff was declined for the same reason the story's own Dev Notes decline the retrofit itself — different order of magnitude, different owner — and after F1's fix, `listings` (the one table `anon` can actually read rows from) itself no longer has this gap.
...
- `supabase/schemas/06_grants.sql` — `listings`/`listings_id_seq` grants; closes the pre-existing `members_id_seq` → `anon` gap (Task 3's "closing a narrower AD-1 gap"); review fixes F1 (name `authenticated` in the `revoke all` alongside `anon`), F6 (`anon`'s SELECT re-declared as an enumerated column grant).
...
- `src/components/atomic-crm/listings/PublishShadchanListingSection.test.tsx` — unit tests (AC-1, AC-2, AC-3, AC-5); review fix F7 (un-publishing a field).
...
- `src/components/atomic-crm/settings/ShadchanListingSection.test.tsx` — gating unit tests; review fix F3 (checks rendered text + `dataProvider.getList` call count instead of a button-only assertion the loading skeleton already satisfied).
...
- `supabase/tests/listings.sql` — database test suite (37 checks, AC-1 through AC-8, including F1/F2/F4/F6 review-fix coverage).
- `supabase/tests/listings.test.ts` — vitest runner for the above.
- `supabase/migrations/20260802224935_listings_review_fixes.sql` — review fixes F1/F4/F6, hand-checked against `db diff` (which caught F1's revokes but not F4's sequence grants or F6's column-level grant — both hand-added, matching the repo's own "`db diff` never re-emits grants" rule).
- `supabase/tests/migration-data-safety/fixture.sql` — review fix F5: `to_regclass('public.listings')`-guarded seed block + `capture('listings')`.
