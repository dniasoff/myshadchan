...
| `make typecheck` / `make test` | **There is no `Makefile`.** `AGENTS.md` documents commands that do not exist. Real: `npm run typecheck` (`package.json:17` = `tsc --noEmit --project tsconfig.app.json && …`). |
