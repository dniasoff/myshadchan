...
- **Rule:** OCR+extraction and reference-question generation call **only** the Cloudflare AI Gateway from a Worker (provider SDK with `baseURL` overridden); the diligence dossier's cross-reference summary ships as a **deterministic** narrative with no model call at all, by design (Story 11.3 adversarial review, Finding 12 — see `workers/ai/dossierNarrator.ts`). **Hebrew OCR:** the parse step uses **Gemini** (Flash default → Pro for hard pages) via the gateway's Google Vertex/AI-Studio provider, returning structured JSON (typed Hebrew+English is well-handled); a **deterministic fallback — Google Document AI / Cloud Vision, a direct disclosed Google Cloud call** — covers degraded typed pages, and **Transkribus/Kraken** the rare handwritten one; generative-OCR **hallucination is guarded** by field validation + low-confidence human review (AD-6/NFR-5). Every call **Langfuse-traced** (`@langfuse/*` OTel SDK); response cache **namespaced by account**. **Hard invariant (FR63):** never judges compatibility / matches; **no outward web-scraping** (enforced by capability). US region; contractual no-training (PRV-6). *(AI tokens are the product's one unavoidable cost — caching + cheap-model-first + batching are **margin-critical** under the $2 cost-recovery model, AD-16.)*
...
| **AI** | ✅ Cloudflare AI Gateway (free) → **Gemini** (OCR+extract; Flash→Pro; Google Vertex/AI-Studio) via gateway `baseURL`; diligence dossier narration ships **deterministic** — no model call, `@anthropic-ai/sdk` removed (Story 11.3 adversarial review, Finding 12); deterministic OCR fallback = Google Document AI/Vision (direct); 💲 model tokens billed by provider · Langfuse `@langfuse/*` 5.9.1 (self-host MIT = free) |
