...
User credentials are stored in Supabase's `auth.users` table. Supabase does not allow to add columns to this table. That's why additional user details are stored in a `members` table created by Atomic CRM. A database trigger is used to automatically sync the `members` record when a user is created or updated (e.g. for the `first_name` and `last_name` fields).
...
Atomic also uses Edge functions for backend logic such as user management.
