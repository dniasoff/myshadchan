...
> **Key insight:** A green heartbeat (`last_ok_at` fresh) does **not** prove anything was delivered. A permanently failing send queue (missing/invalid Resend credentials) and a healthy sweep coexisted in this project's history (Epic 12 adversarial review, R3), which is exactly why `last_failed_count` exists.
...
### (a) Worker Not Running At All — `last_run_at` is Stale
...
**Root causes:**
- Cloudflare cron triggers **not enabled** on deployed Worker (armed in `wrangler.toml` but not in Dashboard)
...
> **UNVERIFIED — confirm with the owner:** Whether Cloudflare cron triggers are actually enabled on the deployed `myshadchan-cron` Worker. The `wrangler.toml` declares them, but Cloudflare Dashboard → Triggers is the ground truth.

---

### (b) Worker Runs, RPCs Succeed, Delivery Fails — `last_ok_at` Fresh, `last_failed_count` > 0
...
**Root causes:**
- **Missing/invalid `RESEND_API_KEY`** in Worker secrets (the concrete case from Epic 12 R3)
...
> **UNVERIFIED — confirm with the owner:** The production `RESEND_API_KEY` and `RESEND_FROM` values in Cloudflare Worker secrets. These are not in the repo and cannot be verified from code.
...
## UNVERIFIED Items (Confirm with Owner)

1. **Cloudflare cron triggers enabled on deployed `myshadchan-cron` Worker** — `wrangler.toml` declares them but Dashboard → Triggers is ground truth.
2. **Production `RESEND_API_KEY` value in Cloudflare Worker secrets** — not in repo, cannot be verified from code.
3. **Production `RESEND_FROM` value in Cloudflare Worker secrets** — must match a verified domain in Resend dashboard.
4. **Production `APP_ORIGIN` value** — `workers/cron/wrangler.toml:75` shows `https://www.myshadchan.space` but Worker secret may differ.
5. **VAPID keys for web push** (`VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`) — only used by Story 7.5 push notifications, not email reminders, but share the same Worker.
