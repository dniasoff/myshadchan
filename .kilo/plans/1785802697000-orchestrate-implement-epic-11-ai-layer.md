...
- [ ] Create `workers/ai/dossierNarrator.ts` (+ `.test.ts`):
  ...
  - Add `@anthropic-ai/sdk` to `package.json` (0.112.x per Stack table).
  - Extend `AiEnv` in `workers/ai/index.ts` with `AI_GATEWAY_ACCOUNT_ID`, `AI_GATEWAY_ID`, `ANTHROPIC_API_KEY`.
...
- [ ] Add `POST /dossier` route in `workers/ai/index.ts`:
  ...
  - Compose narrative via `DossierNarrator` with deterministic fallback on failure/banned phrases.
  - Return the aggregate shape: counts, `covered`, `gaps`, `hasContradiction`, `narrative`.
...
4. **AI Gateway credentials absent locally.** The Gemini/Anthropic extractors and narrators can be unit-tested with fakes; live integration requires `AI_GATEWAY_*` and API keys that may not exist in local `.env` files. Do not block development on live keys.
...
- Are live `GOOGLE_AI_STUDIO_API_KEY` / `ANTHROPIC_API_KEY` / `AI_GATEWAY_*` values available for integration testing, or should the epic ship tested against fakes only?
